...
class _VipScreenState extends State<VipScreen> with TickerProviderStateMixin {
  ...
  final List<_NalaniVipPlan> _vipPlans = [
    _NalaniVipPlan(
      title: '12.99',
      subTitle: 'Per week',
      total: 'Total \$12.99',
      desc: '+7 Days ',
      productId: 'NalaniWeekVIP',
      popular: false,
    ),
    _NalaniVipPlan(
      title: '49.99',
  ...
  Future<void> _loadVipStatus() async {
    ...
    setState(() {
      _isVip = prefs.getBool('nalaniIsVip') ?? prefs.getBool('nalaniIsVip') ?? false;
      final expiryStr = prefs.getString('nalaniVipExpiry') ?? prefs.getString('nalaniVipExpiry');
      _vipExpiry = expiryStr != null ? DateTime.tryParse(expiryStr) : null;
    });
    // Migrate old keys to new keys if exists
    if (prefs.getBool('nalaniIsVip') != null) {
      await prefs.setBool('nalaniIsVip', _isVip);
    }
    if (prefs.getString('nalaniVipExpiry') != null && _vipExpiry != null) {
      await prefs.setString('nalaniVipExpiry', _vipExpiry!.toIso8601String());
    }
  }
  ...
  Future<void> _handleSuccessfulPurchase(PurchaseDetails purchaseDetails) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('nalaniIsVip', true);
    // Calculate expiry date and VIP type
    DateTime now = DateTime.now();
    DateTime expiry;
    String vipType;
    if (purchaseDetails.productID == 'NalaniWeekVIP') {
      expiry = now.add(const Duration(days: 7));
      vipType = 'weekly';
    } else if (purchaseDetails.productID == 'NalaniMonthVIP') {
    ...
    await prefs.setString('nalaniVipExpiry', expiry.toIso8601String());
    await prefs.setString('nalaniVipType', vipType);
    _showSnackBar("Nalani Premium activated!");
  ...
  Future<void> _restorePurchases() async {
    ...
    try {
      ...
      if (_isVip) {
        _showSnackBar("Purchases restored successfully! Nalani Premium activated.");
      } else {
  ...
  Widget _buildSubscriptionCards() {
   ...
   Widget _buildBenefitsList() {
    final List<_NalaniVipPrivilege> privileges = _selectedIndex == 0 ? [
      _NalaniVipPrivilege(icon: Icons.play_circle_outline, text: 'Watch popular dubbing works'),
      _NalaniVipPrivilege(icon: Icons.support_agent, text: 'Ask the ai assistant questions about dubbing'),
      _NalaniVipPrivilege(icon: Icons.block, text: 'Remove advertisements'),
      _NalaniVipPrivilege(icon: Icons.edit, text: 'Customize and modify the avatar')
    ] : [
      _NalaniVipPrivilege(icon: Icons.play_circle_outline, text: 'Watch popular dubbing works'),
      _NalaniVipPrivilege(icon: Icons.support_agent, text: 'Ask the ai assistant questions about dubbing'),
      _NalaniVipPrivilege(icon: Icons.block, text: 'Remove advertisements'),
      _NalaniVipPrivilege(icon: Icons.edit, text: 'Customize and modify the avatar'),
      _NalaniVipPrivilege(icon: Icons.publish, text: 'Freely release dubbing works'), 
    ];
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            GestureDetector(
              onTap: () => _launchURL('https://www.privacypolicies.com/live/deb73d34-3723-4b54-978b-2191bd14f037'),
              child: Text(
                'Privacy Policy',
                style: TextStyle(
                  color: AppTheme.primaryColor,
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                '4. Find "Nalani Premium" and tap it',
                style: TextStyle(
                  color: Colors.grey,
                  fontSize: 14,
                ),
...
class _NalaniVipPrivilege {
  final IconData icon;
  final String text;
  const _NalaniVipPrivilege({required this.icon, required this.text});
}