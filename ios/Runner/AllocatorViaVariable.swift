...
class AllocatorViaVariable {
    //: static let shared = AppPermissionTool()
    static let shared = AllocatorViaVariable()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func total(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func fire(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
    ...
    func objectObserver(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
