...
fileprivate let show_arraySceneTopName:[Character] = ["m","f","/","r","e","c","h","a","r","g","e","/","c","r","e","a","t","e","A","p","p","l","e","P","a","y"]

/*: "productId" :*/
fileprivate let kStartMsg:String = "PRODU"

/*: "source" :*/
fileprivate let notiCountShowKey:String = "SOURCE"

/*: "orderNum" :*/
fileprivate let userPhotoFormat:[Character] = ["o","r","d","e","r","N","u","m"]

/*: "mf/recharge/applePayNotify" :*/
fileprivate let k_carrierMsg:[Character] = ["m","f","/","r","e","c"]
fileprivate let kPrivacyFormat:[Character] = ["h","a","r","g","e","/","a","p","p","l","e","P","a","y","N","o","t","i","f","y"]

/*: "reportMoney" :*/
fileprivate let showActivityTitle:[Character] = ["r","e","p","o","r","t","M"]
fileprivate let mainPurchaseMsg:String = "onefifty"

/*: "transactionId" :*/
fileprivate let kUpId:String = "TRANS"
fileprivate let app_clearProcessFormat:String = "nIdmessage smart permission"

/*: "mf/AutoSub/AppleCreateOrder" :*/
fileprivate let data_languageKey:String = "mf/Augo interval"
fileprivate let show_presentationData:String = "b/Apptrue limit need active"
fileprivate let notiCurrentValue:String = "ainfoe"
fileprivate let appPlatformFormat:String = "Ordershort array again tool"

/*: "orderId" :*/
fileprivate let kActiveTagContent:[UInt8] = [0x64,0x49,0x72,0x65,0x64,0x72,0x6f]

/*: "mf/AutoSub/ApplePaySuccess" :*/
fileprivate let appTotalKey:String = "to white search drop kindmf/A"
fileprivate let const_toData:String = "element prompt transaction result takeb/App"
fileprivate let constPieceMsg:String = "up topSuccess"

/*: "App" :*/
fileprivate let appDecisionMessage:String = "session select method environmentApp"

/*: "OrderTransactionInfo_Cache" :*/
fileprivate let constInstanceText:String = "Orderalbum always any app rating"
fileprivate let appBlackTitle:String = "sacconsequenceio"
fileprivate let k_processFatalTitle:[Character] = ["n","I","n","f","o","_","C","a","c","h","e"]

/*: "OrderTransactionInfo_Subscribe_Cache" :*/
fileprivate let data_errorMessage:[UInt8] = [0x65,0x68,0x63,0x61,0x43,0x5f,0x65,0x62,0x69,0x72,0x63,0x73,0x62,0x75,0x53,0x5f,0x6f,0x66,0x6e,0x49,0x6e,0x6f,0x69,0x74,0x63,0x61,0x73,0x6e,0x61,0x72,0x54,0x72,0x65,0x64,0x72,0x4f]

/*: "verifyData" :*/
fileprivate let user_instanceValue:[UInt8] = [0xc5,0xd6,0xc1,0xda,0xd5,0xca,0xf7,0xd2,0xc7,0xd2]

private func takeCheck(remove num: UInt8) -> UInt8 {
    return num ^ 179
}

/*: " 未知的交易类型" :*/
fileprivate let dataLoadFormat:[Character] = [""," ","\u{672a}","\u{77e5}"]
fileprivate let kNetSearchId:[Character] = ["的","交","\u{6613}","类","型"]
...
enum ApplyUTF8Initializable: String {
    //: case unknow            = "未知类型"
    case unknow = "未知类型"
...
typealias IAPcompletionHandle = (ApplyUTF8Initializable, Double, PriorityType) -> Void

//: class AppleIAPManager: NSObject {
class MethodBoundaryRequestDelegate: NSObject {
    ...
    private var currentPayType: PriorityType = .Pay // 当前支付类型

    // singleton
    //: static let shared = AppleIAPManager()
    static let shared = MethodBoundaryRequestDelegate()
...
private extension MethodBoundaryRequestDelegate {
    /// 【购买】创建业务订单
    /// - Parameters:
    ///   - productId: 产品Id
    ///   - block: 回调
    //: fileprivate func req_pay_createAppleOrder(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
    func component(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = DisabledRequestModel()
        //: reqModel.requestPath = "mf/recharge/createApplePay"
        reqModel.requestPath = (String(show_arraySceneTopName))
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["productId"] = productId
        dict[(kStartMsg.lowercased() + "ctId")] = productId
        //: dict["source"] = source
        dict[(notiCountShowKey.lowercased())] = source
        //: reqModel.params = dict
        reqModel.params = dict
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        TensorCurveVelocity.statisticalProcedure(model: reqModel) { succeed, result, _ in
            ...
            if let value = dict?[(String(userPhotoFormat))] as? String {
                //: orderId = value
                orderId = value
            }
    ...
    func file(_ transactionId: String, params: [String: String]) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = DisabledRequestModel()
        //: reqModel.requestPath = "mf/recharge/applePayNotify"
        reqModel.requestPath = (String(k_carrierMsg) + String(kPrivacyFormat))
        //: reqModel.params = params
        reqModel.params = params
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        TensorCurveVelocity.statisticalProcedure(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    ...
                    self.purchased(transactionId, .Pay)
                }
                //: return
                return
            }
            ...
            let reportMoney: Double = {
                ...
                if let d = dict?[(String(showActivityTitle) + mainPurchaseMsg.replacingOccurrences(of: "fifty", with: "y"))] as? Double { return d }
                //: return 0
                return 0
                //: }()
            }()
            ...
            let newPayCacheList = self.payCacheList.filter { $0[(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3)))] != transactionId }
            //: let diskPath = self.getPayCachePath()
            let diskPath = self.disappear()
...
private extension MethodBoundaryRequestDelegate {
    ...
    func show(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = DisabledRequestModel()
        //: reqModel.requestPath = "mf/AutoSub/AppleCreateOrder"
        reqModel.requestPath = (String(data_languageKey.prefix(5)) + "toSu" + String(show_presentationData.prefix(5)) + "leCre" + notiCurrentValue.replacingOccurrences(of: "info", with: "t") + String(appPlatformFormat.prefix(5)))
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["productId"] = productId
        dict[(kStartMsg.lowercased() + "ctId")] = productId
        ...
        TensorCurveVelocity.statisticalProcedure(model: reqModel) { succeed, result, _ in
            ...
            if let value = dict?[String(bytes: kActiveTagContent.reversed(), encoding: .utf8)!] as? String {
                //: orderId = value
                orderId = value
            }
            //: handle(orderId, succeed)
    ...
    func to(_ transactionId: String, params: [String: String]) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = DisabledRequestModel()
        //: reqModel.requestPath = "mf/AutoSub/ApplePaySuccess"
        reqModel.requestPath = (String(appTotalKey.suffix(4)) + "utoSu" + String(const_toData.suffix(5)) + "lePay" + String(constPieceMsg.suffix(7)))
        //: reqModel.params = params
        reqModel.params = params
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        TensorCurveVelocity.statisticalProcedure(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    ...
                    self.purchased(transactionId, .Subscribe)
                }
                //: return
                return
            }
            ...
            let newSubscribeCacheList = self.subscribeCacheList.filter { $0[(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3)))] != transactionId }
            //: let diskPath = self.getSubscribeCachePath()
            let diskPath = self.fieldDoingTotalry()
...
//: extension AppleIAPManager {
extension MethodBoundaryRequestDelegate {
    /// 初始化数据
    //: private func iap_initData() {
    private func accountingDataTake() {
        //: self.payCacheList = getLocalPayCacheList(payType: .Pay)
        self.payCacheList = atOnce(payType: .Pay)
        //: self.subscribeCacheList = getLocalPayCacheList(payType: .Subscribe)
        self.subscribeCacheList = atOnce(payType: .Subscribe)
    ...
    private func atOnce(payType: PriorityType) -> [[String: String]] {
    ...
    private func disappear() -> String {
        //: let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent((String(appDecisionMessage.suffix(3))))
        ...
        let filePath = (appDirectoryPath as NSString).appendingPathComponent((String(constInstanceText.prefix(5)) + "Tran" + appBlackTitle.replacingOccurrences(of: "consequence", with: "t") + String(k_processFatalTitle)))
        //: return filePath
        return filePath
    }
    ...
    private func fieldDoingTotalry() -> String {
        //: let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent((String(appDecisionMessage.suffix(3))))
        ...
        let filePath = (appDirectoryPath as NSString).appendingPathComponent(String(bytes: data_errorMessage.reversed(), encoding: .utf8)!)
        //: return filePath
        return filePath
    }
    ...
    fileprivate func utilisedObserver(_ transactionId: String, _ payType: PriorityType) -> String? {
        // 有未完成的订单，先取缓存
        ...
        case .Pay:
            ...
            paramsArr = self.payCacheList.filter { $0[(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3)))] == transactionId }
        //: case .Subscribe:
        case .Subscribe:
            //: paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            paramsArr = self.subscribeCacheList.filter { $0[(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3)))] == transactionId }
        }
        //: if paramsArr.count > 0 && paramsArr.first!["verifyData"] != nil {
        if paramsArr.count > 0, paramsArr.first![String(bytes: user_instanceValue.map{takeCheck(remove: $0)}, encoding: .utf8)!] != nil {
            //: return paramsArr.first!["verifyData"]
            return paramsArr.first![String(bytes: user_instanceValue.map{takeCheck(remove: $0)}, encoding: .utf8)!]
        }
...
extension MethodBoundaryRequestDelegate {
    ...
    func from() {
        ...
        for dict in self.payCacheList {
            ...
            oeilDeBoeuf(dict[(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3)))], .Pay)
        }
        ...
        for dict in self.subscribeCacheList {
            ...
            oeilDeBoeuf(dict[(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3)))], .Subscribe)
        }
    }
    ...
    private func oeilDeBoeuf(_ transactionId: String?, _ payType: PriorityType) {
        ...
        purchased(transactionId, payType)
    }
}
...
extension MethodBoundaryRequestDelegate {
    ...
    func begin(productId: String, payType: PriorityType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        //: iap_initData()
        accountingDataTake()
        ...
        case .Pay:
            ...
            component(productId: productId, source: source) { [weak self] orderId, succeed in
    ...
    fileprivate func rangeEmpty(_ productId: String) {
...
extension MethodBoundaryRequestDelegate: SKProductsRequestDelegate {
    // 查询apple内购商品成功回调
...
extension MethodBoundaryRequestDelegate: SKPaymentTransactionObserver {
    /// 2.2 apple内购完成回调
        ...
        for transaction in transactions {
            ...
            case .purchased: // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    self.purchased(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
            ...
            @unknown default:
                ...
                createOrderId = nil
                //: fatalError(" 未知的交易类型")
                fatalError((String(dataLoadFormat) + String(kNetSearchId)))
            }
        }
    }
    ...
    fileprivate func purchased(_ transactionId: String, _ payType: PriorityType) {
        //: guard let receiptStr = getVerifyData(transactionId, payType) else {
        guard let receiptStr = utilisedObserver(transactionId, payType) else {
        ...
        if createOrderId != nil { // 正常支付流程
            ...
            case .Pay:
                ...
                if self.payCacheList.filter({ $0[(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3)))] == transactionId || $0[String(bytes: kActiveTagContent.reversed(), encoding: .utf8)!] == createOrderId }).count == 0 { // 防止重复添加缓存数据
                    //: let cacheDict = ["transactionId": transactionId,
                    let cacheDict = [(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3))): transactionId,
                                     //: "orderId": createOrderId!,
                                     String(bytes: kActiveTagContent.reversed(), encoding: .utf8)!: createOrderId!,
                                     //: "verifyData": receiptStr]
                                     String(bytes: user_instanceValue.map{takeCheck(remove: $0)}, encoding: .utf8)!: receiptStr]
            ...
            case .Subscribe:
                ...
                if self.subscribeCacheList.filter({ $0[(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3)))] == transactionId || $0[String(bytes: kActiveTagContent.reversed(), encoding: .utf8)!] == createOrderId }).count == 0 { // 防止重复添加缓存数据
                    //: let cacheDict = ["transactionId": transactionId,
                    let cacheDict = [(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3))): transactionId,
                                     //: "orderId": createOrderId!,
                                     String(bytes: kActiveTagContent.reversed(), encoding: .utf8)!: createOrderId!,
        ...
        case .Pay:
            ...
            let paramsArr = self.payCacheList.filter { $0[(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3)))] == transactionId }
            //: guard paramsArr.count > 0 else { return }
            guard paramsArr.count > 0 else { return }
            //: req_pay_uploadAppletransaction(transactionId, params: paramsArr.first!)
            file(transactionId, params: paramsArr.first!)

        //: case .Subscribe:
        case .Subscribe:
            //: let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            let paramsArr = self.subscribeCacheList.filter { $0[(kUpId.lowercased() + "actio" + String(app_clearProcessFormat.prefix(3)))] == transactionId }
            //: guard paramsArr.count > 0 else { return }
            guard paramsArr.count > 0 else { return }
            //: req_subscribe_uploadAppletransaction(transactionId, params: paramsArr.first!)
            to(transactionId, params: paramsArr.first!)
        }
    }
}
