...
fileprivate let userCancelStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
let constComplianceData = 80.0
//: let kProgressHUD_cornerRadius = 14.0
let appProgressText = 14.0
//: let kProgressHUD_alpha        = 0.9
let userMaxId = 0.9
//: let kBackgroundView_alpha     = 0.6
let notiEnvironmentMessage = 0.6
//: let kAnimationInterval        = 0.2
let dataContextNetSwitchlyName = 0.2
//: let kTransformScale           = 0.9
let const_limitMsg = 0.9

//: open class ProgressHUD: UIView {
open class RectActivityContrast: UIView {
    //: required public init?(coder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: userCancelStr.reversed(), encoding: .utf8)!)
    }

    //: static var shared = ProgressHUD()
    static var shared = RectActivityContrast()
    ...
    class func user(superView: UIView?) {
        //: if superView != nil {
        if superView != nil {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                //: ProgressHUD.shared.frame = superView!.bounds
                RectActivityContrast.shared.frame = superView!.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                RectActivityContrast.shared.activityIndicator.center = RectActivityContrast.shared.center
                //: superView!.addSubview(ProgressHUD.shared)
                superView!.addSubview(RectActivityContrast.shared)
            }
            //: } else {
        } else {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                ...
                RectActivityContrast.shared.frame = UIScreen.main.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                RectActivityContrast.shared.activityIndicator.center = RectActivityContrast.shared.center
                //: AppConfig.getWindow().addSubview(ProgressHUD.shared)
                SmallIsolateState.worldView().addSubview(RectActivityContrast.shared)
            }
        }
        //: ProgressHUD.shared.hud_startAnimating()
        RectActivityContrast.shared.quantity()
    }

    //: class func dismiss() {
    class func administer() {
        //: ProgressHUD.shared.hud_stopAnimating()
        RectActivityContrast.shared.smart()
    }
    ...
    private func quantity() {
        ...
        DispatchQueue.main.async {
            ...
            self.activityIndicator.transform = CGAffineTransform(scaleX: const_limitMsg, y: const_limitMsg)
            //: self.activityIndicator.alpha = 0
            self.activityIndicator.alpha = 0
            //: UIView.animate(withDuration: kAnimationInterval) {
            UIView.animate(withDuration: dataContextNetSwitchlyName) {
                //: self.backgroundColor = UIColor(white: 0, alpha: kBackgroundView_alpha)
                self.backgroundColor = UIColor(white: 0, alpha: notiEnvironmentMessage)
                //: self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                //: self.activityIndicator.alpha = kProgressHUD_alpha
                self.activityIndicator.alpha = userMaxId
    ...
    private func smart() {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: dataContextNetSwitchlyName) {
                ...
                RectActivityContrast.shared.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.bounds = CGRect(x: 0, y: 0, width: constComplianceData, height: constComplianceData)
        //: indicator.center = self.center
        indicator.center = self.center
        //: indicator.backgroundColor = .black
        indicator.backgroundColor = .black
        //: indicator.layer.cornerRadius = kProgressHUD_cornerRadius
        indicator.layer.cornerRadius = appProgressText
...
extension RectActivityContrast {
    //: class func toast(_ str: String?) {
    class func listInfoAgent(_ str: String?) {
        //: toast(str, showTime: 1)
        ballsUpUser(str, showTime: 1)
    }

    //: class func toast(_ str: String?, showTime: CGFloat) {
    class func ballsUpUser(_ str: String?, showTime: CGFloat) {
        ...
        SmallIsolateState.worldView().addSubview(titleLab)
        ...
        titleLab.center = SmallIsolateState.worldView().center
